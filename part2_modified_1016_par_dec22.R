    ...
    if(Specfile != ''){
          spec_dic <- 
                read.table(paste(dataPath, '\\', Specfile, Ext, sep=''), sep=',', header=T
                           , stringsAsFactors=F)
          idx_spec <- match(x$spec, spec_dic$SPEC_CD)
          specialty <-
    ...
    if(ZIP2Region != ''){
          reg_dic <- 
                read.table(paste(dataPath, '\\', ZIP2Region, Ext, sep=''), sep=',', header=T
                           , stringsAsFactors=F)
          region <-
                ifelse(!is.na(match(x$zipcode, reg_dic$ZIPCODE)), 
                       as.character(reg_dic[match(x$zipcode, reg_dic$ZIPCODE),
                                            "REGION"]), 'OTHER')
    }else{
          region <- 'ALL REGION'
    }   
    
    temp <- cbind(imsdr7=x$imsdr7, specialty= specialty,
...
sfExport('matched_data','othT','othN','nlT', 'nlN'
...
write.xlsx(esti_df, 
           paste(outPath, '\\OutPut_Campaign_', campId, '_v2.xlsx', sep=''), 
